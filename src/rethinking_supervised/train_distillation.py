...
def parse_option():
    ...
    # dataset and model
    parser.add_argument('--model_s', type=str, default='resnet50', choices=model_pool)
    ...
    parser.add_argument('--path_t', type=str, default='./checkpoint/ckpt_epoch_198.pth', help='teacher model snapshot')
    ...
    opt.model_t = 'resnet50'#get_teacher_name(opt.path_t)
...
def load_teacher(model_path, n_cls, dataset='SOFAR'):
...
def main():
    ...
    val_loader = DataLoader(SOFAR(args=opt, partition='train', transform=test_trans),
    ...
    data = torch.randn(2, 3, 32, 32) # image size : 84(miniImagenet). 32(cifar)
    model_t.eval()
    model_s.eval()
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if test_acc > best_acc :
            best_acc = test_acc
            print('==> Saving...')
            save_file = os.path.join('./distil_checkpoint/ckpt_epoch_{epoch}.pth'.format(epoch=epoch))
            torch.save(model_s.state_dict(), save_file)
    
    ## 마지막 파일 저장
    save_file = './distil_checkpoint/last_model.pth'
    torch.save(model_s.state_dict(), save_file)
...
def train(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        acc1 = accuracy(logit_s, target) # prediction, target
